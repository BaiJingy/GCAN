...
from cellvgae import CellVGAE, CellVGAE_Encoder, CellVGAE_GCNEncoder, compute_mmd, GCNGAT, GCNGAT_Encoder
...
def _prepare_training_data(args):
    ...
    if (np.abs(adata.shape[0] - 20000) < 5000) and (adata.shape[0] / adata.shape[1] > 0.4) and not args['transpose_input']:  
        print(colored('WARNING: --transpose_input not provided but input data might have genes in the first dimension. Are you sure you want to continue?', 'yellow'))  
        answer = _user_prompt()  
        if not answer:  
            sys.exit(0)  
    ...
    def _identify_hvg_with_machine_learning(adata, hvg ):  
        X = adata.X  
        cell_means = X.mean(axis=1)  
        threshold = np.mean(cell_means) + np.std(cell_means)  
          
        y_binary = (cell_means > threshold).astype(int)  
        assert y_binary.shape[0] == X.shape[0], "The length of y_binary must be consistent with the number of rows in X."  
        X = np.asarray(X)  
        y_binary = np.asarray(y_binary)
          
        from sklearn.model_selection import cross_val_score  
        scores = cross_val_score(RandomForestClassifier(n_estimators=100, random_state=42), X, y_binary, cv=5, scoring='roc_auc')  
        print(f'Cross-validated AUC for HVG prediction: {np.mean(scores)}')  
        ...
        print(f'Test AUC for HVG prediction: {auc}')  
          
        hvg_idx = np.argsort(clf.predict_proba(X_train_scaled)[:, 1])[::-1][:hvg]  
        adata_hvg = adata[:, hvg_idx]  
        return adata_hvg  

    adata_hvg = _identify_hvg_with_machine_learning(adata_pp, args['hvg'])  
...
def _correlation(data_numpy, k, corr_type='pearson', threshold=0.5):  

    data_std = (data_numpy - np.mean(data_numpy, axis=0)) / np.std(data_numpy, axis=0)  
      
    df = pd.DataFrame(data_std.T)  

    if corr_type == 'spearman':  
        corr1 = df.apply(lambda x: x.rank(method='first').apply(lambda y: spearmanr(x, y)[0]))  
    else:  
        corr1 = df.corr(method=corr_type)  
      
    corr_mask = np.triu(np.ones_like(corr1, dtype=bool), k=1)  
    corr = corr1.where(np.abs(corr1) > threshold).dropna()  
       
    nlargest = k  
    upper_triangle_indices = np.triu_indices_from(corr1, k=1)  
    sorted_indices = np.argsort(-corr.values[upper_triangle_indices])  
    order = sorted_indices.reshape(-1, nlargest) + np.arange(0, corr1.shape[0]*k, k)[:, None]  
...
def _train(model, optimizer, train_data, loss, device, use_decoder_loss=False, conv_type='GAT'):  #原函数
    model = model.train()

    epoch_loss = 0.0
    x, edge_index = train_data.x.to(torch.float).to(device), train_data.edge_index.to(torch.long).to(device)  #原来的
    optimizer.zero_grad()

    if conv_type in ['GAT', 'GATv2','GCNGAT']:
...
def _test(model, device, data, graph_conv):
    model = model.eval()
    if graph_conv in ['GAT', 'GATv2', 'GCNGAT']:
        ...
        #新加
        print(*model.test(z, data.pos_edge_label_index.to(torch.long).to(device), data.neg_edge_label_index.to(torch.long).to(device)), z)
...
def _setup(args, device):
    ...
    if issparse(X_hvg):  
        ...
        scaler = MaxAbsScaler()  
        scaled_x = scaler.fit_transform(X_hvg)  
    else:  
        scaler = MaxAbsScaler()  
        scaled_x = torch.from_numpy(scaler.fit_transform(X_hvg.toarray() if hasattr(X_hvg, 'toarray') else X_hvg)) 
    ...
    if args['graph_convolution'] in ['GAT', 'GATv2']:
        ...
        encoder = CellVGAE_Encoder(
            ...
            concat={'first': True, 'second': True, 'third':True})
        
    if args['graph_convolution'] in ['GCNGAT']:
        num_heads = {}
        if len(args['num_heads']) == 4:
            num_heads['first'] = args['num_heads'][0]
            num_heads['second'] = args['num_heads'][1]
        ...
        encoder = GCNGAT_Encoder(
            ...
            v2=args['graph_convolution'] == 'GATv2',
            use_gcn_first=True,
            concat={'first': True, 'second': True, 'third':True}) #'third':true是新加的
    elif args['graph_convolution'] in ['GCN']:
            encoder = CellVGAE_GCNEncoder(  #encoder
            in_channels=num_features,
            num_hidden_layers=args['num_hidden_layers'],
            hidden_dims=args['hidden_dims'],
            latent_dim=args['latent_dim'])
    else :
        encoderGCN = CellVGAE_GCNEncoder(  #encoder
            in_channels=num_features,
            num_hidden_layers=args['num_hidden_layers'],
            hidden_dims=args['hidden_dims'],
            latent_dim=args['latent_dim'])
    ...
    return model, optimizer, train_data, val_data, test_data, X_hvg      
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_convolution', choices=['GCNGAT'], default='GCNGAT')
    ...
    model, optimizer, train_data, val_data, test_data, X_hvg= _setup(args, device=device)   #X_hvg为新加的
    ...
    epoch_losses = []  
    decoder_losses = [] if args['use_linear_decoder'] else None  
    # Train/val/test code  
    ...
    if args['use_linear_decoder']:  
        plt.plot(decoder_losses, label='NN Decoder Loss')  
    plt.xlabel('Epoch')  
    plt.ylabel('Loss')  
    ...
    loss_name = str((args["input_gene_expression_path"]))
    plt.savefig(loss_name + 'loss.pdf') 
    ...
    if (args['load_model_path'] is not None):
        ...
        if args['graph_convolution'] in ['GAT', 'GATv2','GCNGAT']:
    ...
    if args['name']:
        filename = f'{args["name"]}_GCAN_node_embeddings.npy'
    elif (args['load_model_path'] is None):
        filename = 'GCAN_node_embeddings.npy'
    else:
        filename = 'GCAN_test_node_embeddings.npy'
    ...
    if (args['load_model_path'] is None):
        if args['name']:
            filename = f'{args["name"]}_GCAN_model.pt'
        else:
            filename = 'GCAN_model.pt'
    ...
    if args['umap']:
        ...
        plt.yticks([])
        fig.tight_layout()
        umap_image_save_path = os.path.join(args['model_save_path'], filename + 'UMAP_plot.png')
        plt.savefig(umap_image_save_path, dpi=300)  # 保存为高分辨率 PNG 图片
    ...
    if args['hdbscan']:
        ...
        plt.yticks([])
        fig.tight_layout()
        image_save_path = os.path.join(args['model_save_path'], filename + 'UMAP_HDBSCAN_plots.png')
        plt.savefig(image_save_path, dpi=300)  # 保存为高分辨率 PNG 图片
